...
fn run_gui() -> Result<()> {
    ...
    // Embed the font at compile time so the binary behaves consistently across platforms
    // and packaging setups (no relative-path runtime lookup).
    eframe::run_native(
        "PicoCover",
        options,
        Box::new(|cc| {
            ...
            // Compile-time embed of the TTF so the font is always available in the binary.
            // Ensure the file exists at compile-time at ../assets/NotoSans-Regular.ttf.
